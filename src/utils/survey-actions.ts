...
let mockSurveys: Survey[] = [
  {
    ...
    description:
      'Please complete this survey to help us understand your current health status and goals.',
    questions: [
      {
        id: 'question1',
  ...
  },
  {
    ...
    description:
      'Help us understand your current eating patterns to provide personalized nutrition advice.',
    questions: [
      {
        id: 'question1',
...
export async function createSurvey(
  surveyData: Omit<Survey, 'id' | 'created_at' | 'updated_at'>,
) {
...
export async function updateSurvey(
  id: string,
  surveyData: Partial<Omit<Survey, 'id' | 'created_at' | 'updated_at'>>,
) {
...
export async function deleteSurvey(id: string) {
  ...
  mockAssignments = mockAssignments.filter(
    (assignment) => assignment.surveyId !== id,
  );
...
export async function getAssignmentByUniqueCode(uniqueCode: string) {
  return (
    mockAssignments.find(
      (assignment) => assignment.uniqueCode === uniqueCode,
    ) || null
  );
}
  ...
  mockAssignments[index] = {
    ...
    completedDate:
      status === 'completed'
        ...
        : mockAssignments[index].completedDate,
...
export async function bulkAssignSurvey(
  surveyId: string,
  clientIds: string[],
  dueDate?: string,
) {
...
export async function submitSurveyResponse(
  ...
  try {
    ...
    console.log(
      `Submitting survey responses for assignment ${id} with: ${JSON.stringify(responses)}`,
    );
...
export async function submitPublicSurveyResponse(
  ...
  try {
    ...
    console.log(
      `Submitting public survey responses for survey ${surveyId} with: ${JSON.stringify(responses)}`,
    );
