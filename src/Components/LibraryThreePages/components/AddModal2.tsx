...
import { DoseInfoText, DoseValidationEnglish, MacrosValidationNumber, ValueInfoText, ValueValidation } from "../../../utils/library-unification";
...
const AddModalLibraryTreePages:React.FC<AddModalLibraryTreePagesProps> = ({isOpen,editData,onSubmit,onClose,mode,pageType,loadingCall}) => {
    const placeHolderTitle = () => {
        if(pageType === 'Supplement') return 'Enter supplement title (e.g., Omega-3 Fish Oil)'
        if(pageType === 'Lifestyle') return 'Enter lifestyle title (e.g., Sleep enough)'
                ...
                <div className="flex flex-col justify-between bg-white w-[320px] xs:w-[350px] sm:w-[500px] rounded-[16px] p-6 max-h-[90vh] overflow-y-auto">
                    ...
                    {pageType === 'Lifestyle' && (
                        <MultiTextField
                            label="Value"
                            inputs={[
                                { mode: 'numeric', pattern: '[0-9]*', placeholder: 'Enter value amount', value: formData.value },
                                { mode: 'text', pattern: '*', placeholder: 'Enter unit', value: formData.unit },
                            ]}
                            ...
                            isValid={showValidation ? ValidationForms.IsvalidField('Value', formData.value) : true}
                            validationText={showValidation ? ValidationForms.ValidationText('Value', formData.value) : ""}                        />
                    )}
                    ...
                    {pageType === 'Diet' && (
                        <MultiTextField 
                            label="Macros Goal"
                            inputs={[
                                {mode:'numeric',pattern:'[0-9]*',placeholder:'Carb amount',value:formData.macros.Carbs,label:'Carbs',unit:'(gr)'},
                                {mode:'numeric',pattern:'[0-9]*',placeholder:'Protein amount',value:formData.macros.Protein,label:'Proteins',unit:'(gr)'},
                                {mode:'numeric',pattern:'[0-9]*',placeholder:'Fat amount',value:formData.macros.Fats,label:'Fats',unit:'(gr)'},
                            ]}
                            onchanges={(vales:Array<any>) => {  
                                updateAddData('macros', {
                                    Fats: MacrosValidationNumber(vales[2].value) ? vales[2].value : formData.macros.Fats,
                                    Protein: MacrosValidationNumber(vales[1].value) ? vales[1].value : formData.macros.Protein,
                                    Carbs:  MacrosValidationNumber(vales[0].value) ? vales[0].value : formData.macros.Carbs,                                    
                                });
                            }}  
                            isValid={showValidation ? ValidationForms.IsvalidField('Macros', formData.macros) : true}
                            validationText={showValidation ? ValidationForms.ValidationText('Macros', formData.macros) : ""}
                        />
                    )}
                    ...
                    <div className="flex flex-col mt-4 w-full">
                        ...
                        <RangeCardLibraryThreePages
                            ...
                            isValid={showValidation ? ValidationForms.IsvalidField("Score",formData.score) : true}
                            validationText={showValidation ? ValidationForms.ValidationText('Score', formData.score) : ""}
                        />
                    </div>
