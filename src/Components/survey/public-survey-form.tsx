...
export function PublicSurveyForm({ survey }: PublicSurveyFormProps) {
  ...
  const getQuestionOptions = (question: ApiQuestion): string[] => {
    ...
    if (Array.isArray(question.options)) return question.options.map((opt) => opt.toString())
    return []
  }
  ...
  const currentQuestion =
    currentStep > 0 && currentStep <= sortedQuestions.length ? sortedQuestions[currentStep - 1] : null
  ...
  // Render star rating
  const renderStarRating = (value: string | undefined, max: number, onChange: (value: string) => void) => {
  ...
  const renderEmojiSelection = (value: string | undefined, options: string[], onChange: (value: string) => void) => {
    return (
        ...
        {options.map((emoji, index) => (
          <button
            ...
            className={`text-4xl p-3 rounded-full transition-all ${
              value === emoji ? "bg-green-100 ring-2 ring-green-600 scale-110" : "bg-gray-50"
            } hover:bg-green-50`}
          >
            {emoji}
          </button>
  ...
  const renderScaleSlider = (
    ...
    const currentValue = value ? Number.parseInt(value) : Math.floor((min + max) / 2)
    return (
      <div className="space-y-6 px-4">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-green-600 font-bold text-xl">{currentValue}</span>
  ...
  const renderYesNo = (value: string | undefined, onChange: (value: string) => void) => {
    return (
      <div className="flex justify-center space-x-4">
        <button
          ...
          className={`px-8 py-4 rounded-lg text-lg font-medium transition-all ${
            value === "Yes" ? "bg-green-600 text-white shadow-lg" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
          }`}
        >
          Yes
        </button>
        <button
          ...
          onClick={() => onChange("No")}
          className={`px-8 py-4 rounded-lg text-lg font-medium transition-all ${
            value === "No" ? "bg-red-600 text-white shadow-lg" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
          }`}
        >
          No
        </button>
  ...
  const renderQuestion = (question: ApiQuestion, questionIndex: number) => {
    ...
    switch (questionType) {
          ...
          >
            {questionOptions.map((option, index) => (
              ...
              >
                ...
                />
                <Label htmlFor={`option-${questionIndex}-${index}`} className="flex-grow cursor-pointer font-medium">
                  {option.toString()}
                </Label>
              </div>
            ))}
      ...
      case "star_rating":
        return renderStarRating(response as string, questionOptions.length || 5, (value) => handleResponseChange(value))

      case "emojis":
        return renderEmojiSelection(response as string, questionOptions, (value) => handleResponseChange(value))
      ...
      case "yes_no":
        return renderYesNo(response as string, (value) => handleResponseChange(value))

      default:
        return (
          <div className="p-4 border border-yellow-300 bg-yellow-50 rounded-md">
            <p className="text-yellow-800">Question type "{questionType}" is not supported in this preview.</p>
          </div>
        )
    }
  }
      ...
      <div className="container max-w-4xl mx-auto px-4 py-10">
        <Card className="bg-white shadow-xl border-0 text-center">
          <CardHeader>
            ...
            <CardDescription className="text-lg mt-2">This survey doesn't contain any questions.</CardDescription>
          </CardHeader>
          <CardContent className="pb-8">
            <p className="text-gray-600 mb-6">
              The survey data structure might not be in the expected format. Please check the console for details.
            </p>
            <Button onClick={() => (window.location.href = "/")} variant="outline" size="lg" className="mt-4">
              Return to Home
            </Button>
          </CardContent>
        </Card>
      ...
      {currentStep === 0 && (
        <Card className="bg-white shadow-xl border-0">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-3xl font-bold">{survey.title || "Health & Wellness Survey"}</CardTitle>
          ...
          <CardContent className="text-center">
            <p className="text-gray-600 mb-6">
              This survey contains {sortedQuestions.length} questions and will take approximately{" "}
              {Math.ceil(sortedQuestions.length * 0.5)} minutes to complete.
            </p>
            <p className="text-gray-600 mb-6">
      ...
      {currentQuestion && (
        <Card style={{height:window.innerHeight - 200 + "px"}} className="bg-white shadow-xl h-fit border-0 flex flex-col relative">
          ...
          <CardFooter className="flex justify-between pt-4 w-full ">
            ...
            >
              {currentStep === sortedQuestions.length ? (submitting ? "Submitting..." : "Submit") : "Next"}
            </Button>
          </CardFooter>
        </Card>
      )}
      ...
      {currentStep > sortedQuestions.length && (
        <Card className="bg-white shadow-xl border-0 text-center">
          <CardHeader>
            ...
            <CardDescription className="text-lg mt-2">
              Your response to "{survey.title || "this survey"}" has been submitted successfully.
            </CardDescription>
          </CardHeader>
          <CardContent className="pb-8">
            <p className="text-gray-600 mb-6">
              We appreciate your time and feedback. Your responses will help us improve our services.
            </p>
          </CardContent>
        </Card>
      )}
