...
const mainCondition = (question:any,allQuestions:Array<any>) => {
    let isVisible = false;
    const conditions = question.conditions.sort((a:any,b:any) => a.priority - b.priority);
    const resolveRuleAction = (rule:any) => {
        const checkQuestion = allQuestions.filter((q:any) => q.order === rule.question_order)[0];
        ...
        if(rule.operator === 'between') {
            return checkQuestion.response >= rule.value[0] && checkQuestion.response <= rule.value[1];
        }
        ...
        if(rule.operator === 'contains') {
            return checkQuestion.response.toLowerCase().includes(rule.value.toLowerCase());
        }
        if(rule.operator === 'not_contains') {
            return !checkQuestion.response.toLowerCase().includes(rule.value.toLowerCase());
        }
        if(rule.operator === 'starts_with') {
            return checkQuestion.response.toLowerCase().startsWith(rule.value.toLowerCase());
        }
        if(rule.operator === 'ends_with') {
            return checkQuestion.response.toLowerCase().endsWith(rule.value.toLowerCase());
        }
        ...
        if(rule.operator === 'all_in_array') {
            return rule.value.every((item:any) => checkQuestion.response.includes(item));
        }
        if(rule.operator === 'any_in_array') {
            return rule.value.some((item:any) => checkQuestion.response.includes(item));
        }
        // LENGTH OPERATORS
        ...
        if(rule.operator === 'file_uploaded') {
            return checkQuestion.response && checkQuestion.response !== null && checkQuestion.response !== '';
        }
        ...
        if(rule.operator === 'file_size_greater') {
            ...
            const fileSize = checkQuestion.response?.size || checkQuestion.response?.fileSize || 0;
