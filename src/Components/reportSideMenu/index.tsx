...
const ReportSideMenu: React.FC<ReportSideMenuProps> = ({
  ...
  const resolveSteps = () => {
    const items = activeTab === 'Progress' ? progressMenuItems : healthMenuItems;
    if (!isShare) {
      return items;
    } else {
      return items.filter(
  ...
  useEffect(() => {
    ...
    const handleScrolledSection = (data: any) => {
      ...
      const currentItems = activeTab === 'Progress' ? progressMenuItems : healthMenuItems;
    ...
    publish('activeTabChange', { tab: activeTab });
  ...
  const onchangeMenu = (item: string) => {
    ...
    if (activeTab === 'Progress') {
      ...
      const elementId = item === 'Wellness Data' ? 'wellness-summary' : 'score-progression';
      ...
      <div className="flex gap-1 mb-4">
        ...
        >
          <img
            ...
            alt="Health"
            className="w-4 h-4"
            style={activeTab === 'Health' ? { filter: 'brightness(0) invert(1)' } : { filter: 'opacity(0.5) brightness(0) saturate(0)' }}
          />
        ...
        >
          <img
            ...
            alt="Progress"
            className="w-4 h-4"
            style={activeTab === 'Progress' ? { filter: 'brightness(0) invert(1)' } : { filter: 'opacity(0.5) brightness(0) saturate(0)' }}
          />
