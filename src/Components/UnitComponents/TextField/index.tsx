...
const TextField:React.FC<TextFieldProps> = ({label, placeholder, onChange, value, isValid, validationText}) => {
  return (
    <>
          <div className="flex flex-col mt-5 w-full gap-2">
            <label htmlFor={"text-field-"+label.toLowerCase().replace(/\s+/g, '-')}  className="text-xs font-medium text-Text-Primary">{label}</label>
            <input
