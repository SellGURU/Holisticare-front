            ...
            <div onClick={() => {
                ...
                <div className="flex">
                    <div className="w-[72px] h-[72px] overflow-hidden rounded-full object-cover">
                        <img className="w-full h-full" onError={(e:any) => {
                            e.target.src = `https://ui-avatars.com/api/?name=${client.name}`; // Set fallback image
                        }} src={client.picture?client.picture:`https://ui-avatars.com/api/?name=${client.name}`} alt="" />
                    </div>
                    <div className="pl-2 grid grid-cols-1 gap-1 ">
                        <div className="text-Text-Primary text-[14px] font-medium">{client.name}</div>
                        <div className="text-Text-Secondary text-[12px]">{client.age} years </div>
