...
const QuestionRow: FC<QuestionRowProps> = ({ el, member_id, onAssign ,handleCloseSlideOutPanel}) => {
    const isActive =() => {
    ...
    const resolveStatusName =() => {
        if(el.status == 'being_entered' || el.status == 'being_deleted' || el.status == 'being_edited') {
            return 'completed'
        }
        return el.status
    }
        ...
        >
            ...
            status={resolveStatusName()}
             ...
             onDelete={() => {
              ...
              />}
            <div
            className={`flex justify-between items-center w-full ${!isActive()? 'opacity-50' : ''}`}
            >
            ...
            <>
                ...
                <div className="text-[10px] text-Text-Quadruple mt-2 leading-5">
                    If you'd like, you may continue working while the system updates your changes.
                </div>
                </div>
            </>           
            </>
