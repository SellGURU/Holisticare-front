...
// Removed yupResolver and yup import
import SpinnerLoader from '../../Components/SpinnerLoader'; // Adjust path as needed
import ThresholdRangesEditor from './ThresholdRangesEditor'; // Ensure this path is correct
...
import {
  ApiBiomarkerData,
  FormBiomarkerData,
  ApiThresholdRange, // Import ApiThresholdRange for transformation
} from '../../types/biormarker'; // Adjust path as needed

interface AddModalProps {
  data: Partial<ApiBiomarkerData>; // Initial data from API, can be partial
  onCancel: () => void;
  onSave: (values: ApiBiomarkerData) => void; // onSave expects API format
  loading: boolean;
  errorDetails: string;
...
const AddModal: FC<AddModalProps> = ({
  ...
  const {
    ...
    // NO RESOLVER NEEDED! React Hook Form will use default validation (e.g., 'required')
    defaultValues: {
      ...
      thresholds: {
        male: Object.entries(data.thresholds?.male || {}).map(
          ([ageRange, ranges]) => ({
            ageRange,
            // Ensure ranges is an array of ApiThresholdRange
    ...
    } as FormBiomarkerData, // Explicitly cast to FormBiomarkerData for type safety
  });
  ...
  // Explicitly type onSubmit handler
  const onSubmit: SubmitHandler<FormBiomarkerData> = (formValues) => {
    setErrorDetails('');

    // Transform formValues (FormBiomarkerData) back to ApiBiomarkerData format
    const transformedValues: ApiBiomarkerData = {
      'Benchmark areas': formValues['Benchmark areas'], // Ensure required fields are handled
      Biomarker: formValues.Biomarker, // Ensure required fields are handled
      Definition: formValues.Definition || undefined, // Use undefined if empty string
      unit: formValues.unit || undefined, // Use undefined if empty string
    ...
    onSave(transformedValues); // Call onSave with API format
  };
