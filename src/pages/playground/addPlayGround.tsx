...
const AddPlayGround: FC<AddPlayGroundProps> = ({ isOpen, onClose, onSubmited }) => {
    ...
    if (isOpen) {
      ...
      Application.getInputPlayGround()
        .then((res) => {
          ...
          const initialValues = res.data.reduce((acc: Record<string, any>, field: FieldSchema) => {
  ...
  return (
    <MainModal isOpen={isOpen} onClose={onClose}>
      <div className="w-[450px] relative min-h-[450px] bg-white rounded-2xl p-6 flex flex-col">
        <h2 className="text-[14px] font-medium text-Text-Primary">Add Playground</h2>

        {loading ? (
          <p className="text-gray-500">Loading...</p>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4 hidden-scrollbar flex-1 overflow-y-auto">
            {schema.map((field) => (
              <div key={field.field_name} className="space-y-1">
                ...
                {field.type === "list"  && (
                    <SelectBoxField label={field.field_name} options={field.options?field.options:variables} value={formValues[field.field_name] || ""} onChange={(e) => handleChange(field.field_name, e)} placeholder={field.field_name}>      
                    </SelectBoxField>
                )}
