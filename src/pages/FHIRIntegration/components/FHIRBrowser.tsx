...
const FHIRBrowser: React.FC<FHIRBrowserProps> = ({ server, onBack }) => {
  ...
  const handleConfirmCreatePatient = async () => {
    ...
    try {
      ...
      if (!createResponse.data.success) {
        throw new Error(
          createResponse.data.message || 'Failed to create patient',
        );
      }
  ...
  const handleBiomarkersNext = async () => {
    // If no biomarkers, just proceed
    ...
    try {
      ...
      // Use FHIR-specific validate endpoint (doesn't require file_id)
      await FHIRApi.validateBiomarkers({
        member_id: createdMemberId,
        biomarkers: mappedBiomarkers,
        date_of_test: formattedDate,
      });
      ...
      if (detail) {
        ...
        // Set row errors from modified_biomarkers_list (FHIR validate returns this format)
        const validationErrors: Record<number, string> = {};
        parsedDetail.modified_biomarkers_list?.forEach((item: any) => {
          validationErrors[item.index] = item.detail;
        });
  ...
  const handleSaveBiomarkers = async () => {
    ...
    try {
      ...
      // Use SaveLabReport with added_biomarkers structure (same as index.tsx)
      const response = await Application.SaveLabReport({
        member_id: createdMemberId,
        modified_biomarkers: {
          biomarkers_list: [], // Empty - no modified biomarkers for FHIR import
          date_of_test: addedTimestamp,
          lab_type: 'more_info',
          file_id: '', // No file_id for FHIR import
        },
        added_biomarkers: {
          biomarkers_list: mappedBiomarkers, // FHIR biomarkers as added
          date_of_test: addedTimestamp,
          lab_type: 'more_info',
        },
      });
        ...
        {steps.map((s, i) => (
          <React.Fragment key={s.key}>
            <div className="flex items-center">
              ...
              <span
                className={`ml-2 text-sm ${i === currentIndex ? 'font-medium text-gray-800' : 'text-gray-500'}`}
              >
                {s.label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div
                className={`w-12 h-0.5 mx-2 ${i < currentIndex ? 'bg-green-500' : 'bg-gray-200'}`}
              />
            )}
          </React.Fragment>
        ))}
          ...
          <div className="divide-y max-h-96 overflow-y-auto">
            {patients
              .filter((patient) => {
                ...
                return (
                  counts && (counts.observations > 0 || counts.conditions > 0)
                );
              })
              .map((patient) => {
                const counts = patientCounts[patient.id];
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500">
                        {patient.gender} | DOB:{' '}
                        {formatFHIRDate(patient.birth_date)} | ID: {patient.id}
                      </p>
                      {counts && (
                        <div className="flex gap-3 mt-1">
                          <span
                            className={`text-xs ${counts.observations > 0 ? 'text-blue-600 font-medium' : 'text-gray-400'}`}
                          >
                            {counts.observations} observation
                            {counts.observations !== 1 ? 's' : ''}
                          </span>
                          <span
                            className={`text-xs ${counts.conditions > 0 ? 'text-purple-600 font-medium' : 'text-gray-400'}`}
                          >
                            {counts.conditions} condition
                            {counts.conditions !== 1 ? 's' : ''}
                          </span>
                        </div>
                      )}
                    </div>
            ...
            <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-xl">
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                ...
                <div className="space-y-1 text-sm">
                  <p>
                    <strong>Name:</strong>{' '}
                    {patientFormData.patient_data.first_name}{' '}
                    {patientFormData.patient_data.last_name}
                  </p>
                  <p>
                    <strong>Email:</strong> {patientFormData.patient_data.email}
                  </p>
                  <p>
                    <strong>DOB:</strong>{' '}
                    {patientFormData.patient_data.date_of_birth}
                  </p>
                  <p>
                    <strong>Gender:</strong>{' '}
                    {patientFormData.patient_data.gender}
                  </p>
                  {patientFormData.conditions.length > 0 && (
                    <p>
                      <strong>Conditions:</strong>{' '}
                      {patientFormData.conditions.length}
                    </p>
                  )}
                  {patientFormData.medications.length > 0 && (
                    <p>
                      <strong>Medications:</strong>{' '}
                      {patientFormData.medications.length}
                    </p>
                  )}
                  {patientFormData.allergies.length > 0 && (
                    <p>
                      <strong>Allergies:</strong>{' '}
                      {patientFormData.allergies.length}
                    </p>
                  )}
                </div>
              </div>
              ...
              <p className="text-sm text-gray-500 mb-4">
                This will create the patient in Holisticare. You can then review
                and import their biomarkers.
              </p>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-blue-700">
              <strong>{biomarkers.length} biomarker(s)</strong> loaded from
              FHIR. Review and edit the values below.
            </span>
          </div>
        </div>
      )}
        ...
        {biomarkers.length > 0 ? (
          ...
          <div className="p-8 text-center text-gray-500">
            ...
            <p className="text-sm mt-2">
              You can still continue to import the patient without biomarkers.
            </p>
          </div>
        )}
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
            ...
            <p className="text-green-700">
              Patient data has been imported into Holisticare
            </p>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Review the summary and save biomarkers to the patient's profile
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-green-700">
                {patientFormData?.patient_data.first_name}{' '}
                {patientFormData?.patient_data.last_name} (
                {patientFormData?.patient_data.email})
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="text-sm text-gray-600 space-y-1">
                ...
                <p>
                  <strong>Biomarkers to save:</strong>{' '}
                  {biomarkers.length > 0 ? (
                    biomarkers.length
                  ) : (
                ...
                {biomarkers.length > 0 && (
                  <p>
                    ...
                    {dateOfTest?.toLocaleDateString() || 'Today'}
                  </p>
                )}
              </div>
            </div>
