...
const SuperSetExersiseItem:React.FC<SuperSetExersiseItemProps> = ({index,exercise,onDelete,toSuperSet,onChange}) => {
  return (
    <>
    <div className="flex flex-col ml-2 gap-2 relative">
        {exercise.Exercises.map((el:any) => (
            <ExerciseItem
                onDelete={onDelete}
                key={index}
                index={index}
        ...
        <div className="absolute z-[1] h-[85%] top-[25px] left-[-8px]">
           <div className="w-[20px] h-full rounded-[16px]  bg-white border-2 border-gray-300 border-r-white">
