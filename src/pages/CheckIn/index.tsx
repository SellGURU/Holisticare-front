...
const Checkin = () => {
  ...
  const resolveQuestionCard =(item:any) => {
    switch(item.type) {
      case "yes/no": return (<YesNoCard question={item.question} value={item.value}></YesNoCard>)
      case "range": return (<RangeCard question={item.question} value={item.value}></RangeCard>)
    }
  }
  return (
    <>
