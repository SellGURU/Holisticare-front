...
const CheckInControllerModal:React.FC<CheckInControllerModalProps> =({mode,onClose}) => {
    const [questions,setQuestions] = useState<Array<checkinType>>([])
    const resolveFormTitle =() => {
        switch(mode) {
            case "Add": return 'Create a Check-In'
            case "Reposition": return 'Reposition Check-in'
            case "Edit":return 'Edit'
        }
    }
    const resolveBoxRender = () => {
        switch(mode) {
            case "Add": return <AddCheckIn onChange={(values) => {
              setQuestions(values)
            }}></AddCheckIn>
            case "Reposition": return 'Reposition Check-in'
            case "Edit":return 'Edit'
            ...
            {questions?.map((item: any,index:number) => {
              return (
                <>
                  ...
                  onRemove={() => {
                    setQuestions((pre) => {
                      const newQuestions = pre.filter((_el,ind) => ind != index)
                      return newQuestions
                    })
                  }}
                  index={index} 
