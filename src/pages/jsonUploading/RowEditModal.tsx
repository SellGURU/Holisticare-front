    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center">
      ...
      <div className="relative w-full h-full md:h-auto md:max-h-[85vh] md:w-[92vw] md:rounded-2xl bg-white shadow-xl overflow-hidden flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {columns.map((k) => {
              ...
              return (
                <div
                  key={String(k)}
                  className={cn('rounded-xl border border-slate-200 bg-white p-3')}
                >
                  ...
                  {err && (
                    <div className="mt-1 text-xs text-red-600">{String(err)}</div>
                  )}
                </div>
              );
            })}
        ...
        <div className="border-t border-slate-200 p-4 flex items-center justify-between">
          <div className="text-xs text-slate-500">
            Tip: complex fields can be edited as JSON (if you didn’t provide a custom renderer).
          </div>
